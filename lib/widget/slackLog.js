...
function SlackLog(options) {
  ...
  this.os = os.platform()
  this.macNotifications = os.platform() == 'darwin' && parseFloat((os.release()).match(/^([\d]*\.\d)/)) > 10.9
...
SlackLog.prototype.logMessage = function(message, sendNotifications=true) {
  ...
  if (sendNotifications && message.text.includes(this.currentUser)) {
    if (this.macNotifications) {
      child.exec(`osascript -e 'display notification "${message.text}" with title "New Mention in Slack"'`)
    }
  }
