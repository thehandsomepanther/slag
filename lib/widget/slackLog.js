...
function SlackLog(options) {
  ...
  this.notifTriggers = new RegExp(['!channel', '!here', this.currentUser].join('|'))
...
SlackLog.prototype.logMessage = function(message, sendNotifications=true) {
        ...
        if (message.attachments != undefined) {
          ...
          for (let attachment of message.attachments) {
            if (attachment.fallback) {
              chatmessage = _.concat(chatmessage, this.wrap(parseMessage(attachment.fallback, this.userList, this.channelList)).split('\n'))
            }
          }
        }
        break
  ...
  if (sendNotifications && message.text != undefined && this.notifTriggers.test(message.text)) {
    notifier.notify({
      title: `#${this.channelList[message.channel]}`,
      message: message.text.replace(this.currentUser, this.userList[this.currentUser])
    })
  }
